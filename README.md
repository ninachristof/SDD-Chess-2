...
To run the program, type "python main.py <host/connect> <ip> <port>". For example, one player will host the game on their computer, so they might have "python main.py host 0.0.0.0 2020", while the other player will join the game, so they will type "python main.py connect <ip> 2020" where <ip> is the ipv4 address of the first player/computer.
