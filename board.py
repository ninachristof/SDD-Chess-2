...
class board:
    ...
    def add_piece(self, x, y, piece, color):
        ...
        if (self.chess_array[x][y] != None):
            print(f"The {color} {piece} piece cannot be placed at ({x}, {y}) because there is already a {self.chess_array[x][y].get_color()} {self.chess_array[x][y].get_name()} piece there")
            return
        ...
        if color == "white":
            ...
            if piece == 'k': self.white_king_xy = (x, y)
        else:
            self.black_pieces.append((x, y))
            if piece == 'k': self.black_king_xy = (x, y)
        ...
        for i in range(8):
            ...
            for j in range(8):
                if (self.chess_array[i][j] != None):
                    print("{0:4}".format(self.chess_array[i][j].get_name() + " " + self.chess_array[i][j].get_color()[0]), end="|")
                else:
                    print("   ", end=" |")
            print("\n  " + "-" * 41)
    ...
    def get_possible_moves(self,x,y,color):
        ...
        if (self.chess_array[x][y].get_is_debuffed()):
            possible_moves2 = self.chess_array[x][y].apply_debuff(possible_moves2)
