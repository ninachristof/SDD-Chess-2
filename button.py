    ...
    def draw(self, screen):
        ...
        if not self.box.collidepoint(pos) or not self.hover:
            pygame.draw.rect(screen, self.color, self.box,
                             width=0, border_radius=2)
        else:
            darker_color = (
                max(self.color[0]-20, 0), max(self.color[1]-20, 0), max(self.color[2]-20, 0))
            pygame.draw.rect(screen, darker_color, self.box,
                             width=0, border_radius=2)
...
class image_button(Button):
    ...
    def __init__(self, x, y, width, height, filename, scale, callback, *args, **kwargs):
        ...
        self.sprite = pygame.transform.scale(
            pygame.image.load(filename), (width*scale, height*scale))
