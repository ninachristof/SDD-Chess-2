...
# running this function on a separate thread
    ...
    # PIECE PROMOTIONN
        ...
        if upgrade != "" and mpiece != "":
            ...
            self.board.chess_array[mx][my].upgrades = [
                modifier.get_capture(), modifier.get_move()]
    ...
    def select_square(self, i, j):
        ...
        # Otherwise, you are attempting to make a move; see if this move is possible, and do it if so
            ...
            if wanted_move_xy in valid_moves:
                ...
                self.current_instruction = struct.pack(
                    "iiii5s", i, j, current_x, current_y, bytes(self.board.my_color, "utf-8"))
    ...
    def upgrade_callback(self, modifierData):
        ...
        print(self.board.chess_array[randomPiece[0]][randomPiece[1]].get_name(
        ), " at ", randomPiece[0], ",", randomPiece[1], " is getting modified")
        ...
        if (self.board.chess_array[x][y].get_color() == self.board.my_color):
            self.board.chess_array[x][y].upgrades = [
                modifier.get_capture(), modifier.get_move()]
            ...
            self.board.chess_array[randomPiece[0]
                                   ][randomPiece[1]].set_debuff(modifier)
            # TODO: this is temp
        ...
        if (len(self.modifiers) == 0):
            ...
            for i in range(4):
                ...
                if (self.board.chess_array[x][y].get_color() == self.board.my_color):
                    ...
                    powerup = modifiers.get_powerups(
                        self.board.chess_array[x][y].get_name())
                    powerupdescription = "Your " + self.board.chess_array[x][y].get_name(
                    ) + " at " + chr(ord("a") + y) + str(8 - x) + powerup.get_description()
                    # print("Power up ", i, " - " , powerupdescription)
                    self.modifiers.append(
                        (randomPiece, powerup, powerupdescription, 0))
                else:
                    ...
                    debuffdescription = "Your opponent's " + self.board.chess_array[x][y].get_name(
                    ) + " at " + chr(ord("a") + y) + str(8 - x) + debuff.get_description()
                    # print("Debuff ", i, " - " , debuffdescription)
                    self.modifiers.append(
                        (randomPiece, debuff, debuffdescription, idx))

        offset = 10
        for i in range(4):
            randomPiece, modifier, description, idx = self.modifiers[i]
            button = text_button(color, (HEIGHT * 0.8 + 5), (offset*5 + HEIGHT * i * 0.125 + (i * offset)),
                                 (HEIGHT * 0.2 - 10), (HEIGHT * 0.125), 15, description, self.upgrade_callback, self.modifiers[i])
        ...
        if (self.clicked_square != None):
            ...
            if self.board.my_color == "white" and (((8 * current_x) + (current_y)) + (current_x % 2)) % 2 == 0:
                pygame.draw.rect(self.screen, gray, [
                                 (current_y * (HEIGHT * 0.1)), current_x * (HEIGHT * 0.1), (HEIGHT * 0.1), (HEIGHT * 0.1)])
            elif self.board.my_color == "white":
                pygame.draw.rect(self.screen, green, [
                                 (current_y * (HEIGHT * 0.1)), current_x * (HEIGHT * 0.1), (HEIGHT * 0.1), (HEIGHT * 0.1)])
            if self.board.my_color == "black" and (((8 * current_x) + (current_y)) + (current_x % 2)) % 2 == 0:
                pygame.draw.rect(self.screen, gray, [
                                 ((7-current_y) * (HEIGHT * 0.1)), (7-current_x) * (HEIGHT * 0.1), (HEIGHT * 0.1), (HEIGHT * 0.1)])
            ...
            for move in valid_moves:
                ...
                if (self.board.my_color == "white"):
                    if (((8 * move[0]) + (move[1])) + (move[0] % 2)) % 2 == 0:
                        pygame.draw.circle(self.screen, gray, [((move[1] * (HEIGHT * 0.1)) + (
                            HEIGHT * 0.1)/2), move[0] * (HEIGHT * 0.1) + (HEIGHT * 0.1)/2], 30)
                    else:
                        pygame.draw.circle(self.screen, green, [((move[1] * (HEIGHT * 0.1)) + (
                            HEIGHT * 0.1)/2), move[0] * (HEIGHT * 0.1) + (HEIGHT * 0.1)/2], 30)
                if (self.board.my_color == "black"):
                    if (((8 * move[0]) + (move[1])) + (move[0] % 2)) % 2 == 0:
                        pygame.draw.circle(self.screen, gray, [(((7-move[1]) * (HEIGHT * 0.1)) + (
                            HEIGHT * 0.1)/2), (7-move[0]) * (HEIGHT * 0.1) + (HEIGHT * 0.1)/2], 30)
                    else:
                        # pygame.draw.rect(self.screen, green, [ ((7-move[1]) * (HEIGHT * 0.1) ), (7 - move[0]) * (HEIGHT * 0.1),(HEIGHT * 0.1) ,(HEIGHT * 0.1) ])
                        pygame.draw.circle(self.screen, green, [(((7-move[1]) * (HEIGHT * 0.1)) + (
                            HEIGHT * 0.1)/2), (7-move[0]) * (HEIGHT * 0.1) + (HEIGHT * 0.1)/2], 30)
    ...
    def draw_selected_info(self):
        if (self.clicked_square != None):
            ...
            piece_object = self.board.get_square(current_x, current_y)
            # bro why is this inverted x should always horizontal
            self.screen.blit(piece_object.sprite, (HEIGHT * 0.9, HEIGHT * 0.7))
            upgrades = "Upgrades: "
            debuffs = "Debuffs: "
            ...
            if (piece_object.get_is_debuffed()):
                ...
                debuffs += "None"
            button = text_button((255, 255, 255), 0, (HEIGHT * 0.8),
                                 (HEIGHT * 0.8), (HEIGHT * 0.1), 15, upgrades, None)
            button.hover = False
            button.draw(self.screen)
            button = text_button((255, 255, 255), 0, (HEIGHT * 0.9),
                                 (HEIGHT * 0.8), (HEIGHT * 0.1), 15, debuffs, None)
            button.hover = False
            button.draw(self.screen)
        ...
        for i in range(8):
            if self.board.my_color == "black":
                ...
                rect = text_surf.get_rect(
                    center=(10, HEIGHT*.1 * i + (HEIGHT*0.015)))
                self.screen.blit(text_surf, rect)

                text_surf = font.render(str(chr(ord("h") - i)), True, "black")
                rect = text_surf.get_rect(
                    center=(WIDTH*.1 * i - 10 + WIDTH*.1, HEIGHT*.8 - 10))
                self.screen.blit(text_surf, rect)
            if self.board.my_color == "white":
    ...
    def draw_pieces(self):
        uparrow = pygame.transform.scale(
            pygame.image.load("resources/uparrow.png"), (30, 30))
        downarrow = pygame.transform.scale(
            pygame.image.load("resources/downarrow.png"), (30, 30))
        if (self.board.my_color == "white"):
            ...
            for x, y in self.board.white_pieces:
                ...
                # bro why is this inverted x should always horizontal
                self.screen.blit(
                    piece.sprite, (y * (HEIGHT * 0.1), x * (HEIGHT * 0.1)))
                if (piece.get_is_upgraded()):
                    # bro why is this inverted x should always horizontal
                    self.screen.blit(
                        uparrow, (y * (HEIGHT * 0.1) + 50, x * (HEIGHT * 0.1)))
                if (piece.get_is_debuffed()):
                    # bro why is this inverted x should always horizontal
                    self.screen.blit(
                        downarrow, (y * (HEIGHT * 0.1), x * (HEIGHT * 0.1)))
            ...
            for i in range(len(self.board.white_pieces)):
                ...
                # bro why is this inverted x should always horizontal
                self.screen.blit(
                    piece.sprite, ((7-y) * (HEIGHT * 0.1), (7-x) * (HEIGHT * 0.1)))
                if (piece.get_is_upgraded()):
                    # bro why is this inverted x should always horizontal
                    self.screen.blit(
                        uparrow,  ((7-y) * (HEIGHT * 0.1) + 50, (7-x) * (HEIGHT * 0.1)))
                if (piece.get_is_debuffed()):
                    # bro why is this inverted x should always horizontal
                    self.screen.blit(
                        downarrow,  ((7-y) * (HEIGHT * 0.1), (7-x) * (HEIGHT * 0.1)))
    ...
    def draw_board(self):
        for i in range(32):
            ...
            if row % 2 == 0:
                ...
                pygame.draw.rect(self.screen, color, [(HEIGHT * 0.6) - (column * (
                    HEIGHT * 0.2)), row * (HEIGHT * 0.1), (HEIGHT * 0.1), (HEIGHT * 0.1)])
            else:
                pygame.draw.rect(self.screen, color, [(HEIGHT * 0.7) - (column * (
                    HEIGHT * 0.2)), row * (HEIGHT * 0.1), (HEIGHT * 0.1), (HEIGHT * 0.1)])
            button = text_button((255, 255, 255), 0, (HEIGHT * 0.8),
                                 (HEIGHT * 0.8), (HEIGHT * 0.1), 15, "", None)
            button.hover = False
            button.draw(self.screen)
            button = text_button((255, 255, 255), 0, (HEIGHT * 0.9),
                                 (HEIGHT * 0.8), (HEIGHT * 0.1), 15, "", None)
            ...
            self.draw_valid()
            for i in range(9):
                pygame.draw.line(self.screen, 'black', (0, (HEIGHT * 0.1)
                                 * i), ((HEIGHT * 0.8), (HEIGHT * 0.1) * i), 2)
                pygame.draw.line(self.screen, 'black', ((
                    HEIGHT * 0.1) * i, 0), ((HEIGHT * 0.1) * i, (HEIGHT * 0.8)), 2)
        red = (255, 0, 0)
        if (self.white_king_in_check):
            x, y = self.board.get_king_location("white")
            if (self.board.my_color == "white"):
                pygame.draw.circle(self.screen, red, [(
                    (y * (HEIGHT * 0.1)) + (HEIGHT * 0.1)/2), x * (HEIGHT * 0.1) + (HEIGHT * 0.1)/2], 30)
            if (self.board.my_color == "black"):
                pygame.draw.circle(self.screen, red, [(
                    ((7-y) * (HEIGHT * 0.1)) + (HEIGHT * 0.1)/2), (7-x) * (HEIGHT * 0.1) + (HEIGHT * 0.1)/2], 30)
        if (self.black_king_in_check):
    ...
    def draw_promotion_options(self):
        ...
        if (self.board.my_color == "black"):
            options = ["resources/Chess_ndt60.png", "resources/Chess_rdt60.png",
                       "resources/Chess_bdt60.png", "resources/Chess_qdt60.png"]
            n = ["kn", "r", "b", "q"]
            for k in range(2, 6):
                piece_background = text_button(
                    (150, 150, 150), (HEIGHT * 0.85), (HEIGHT * k * 0.1 + offset * k), (HEIGHT * 0.1), (HEIGHT * 0.1), 0, "", None)
                piece_background.draw(self.screen)
                piece_promote = image_button((HEIGHT * 0.85), (HEIGHT * k * 0.1 + offset * k),
                                             (HEIGHT * 0.1), (HEIGHT * 0.1), options[k-2], 1, self.promote, i, j, n[k-2])
                piece_promote.draw(self.screen)
        if (self.board.my_color == "white"):
            options = ["resources/Chess_nlt60.png", "resources/Chess_rlt60.png",
                       "resources/Chess_blt60.png", "resources/Chess_qlt60.png"]
            n = ["kn", "r", "b", "q"]
            for k in range(2, 6):
                piece_promote = image_button((HEIGHT * 0.8), (HEIGHT * k * 0.1), (HEIGHT * 0.1),
                                             (HEIGHT * 0.1), options[k-2], 1, self.promote, i, j, n[k-2])
                piece_promote.draw(self.screen)
        return True
    ...
    def main_loop(self):
        while self.running:
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and self.endgame == "":
                    if (event.pos[0] < 0.8 * WIDTH and event.pos[1] < 0.8 * WIDTH):
                        if (self.board.my_color == "white"):
                            self.select_square(
                                event.pos[1] // (WIDTH // 10), event.pos[0] // (WIDTH // 10))
                        else:
                            self.select_square(
                                7 - event.pos[1] // (WIDTH // 10), 7 - event.pos[0] // (WIDTH // 10))
            ...
            if self.endgame != "":
                color = (105, 194, 250)
                button = text_button(color, (HEIGHT * 0.2), (HEIGHT * 0.3),
                                     (HEIGHT * 0.4), (HEIGHT * 0.2), 50, self.endgame, None)
                button.hover = False
                button.draw(self.screen)
...
# for main menu
        ...
        height_offset = 50
        host_button = image_button(button_x_pos, HEIGHT//2 - height_offset,
                                   57, 9, "resources/create_game_button.png", scale, None)
        join_button = image_button(button_x_pos, HEIGHT//2 + height_offset,
                                   57, 9, "resources/join_game_button.png", scale, None)
        textbox_width = 350
        textbox_height = 50
# for joining game menu
        # TODO:add color pallete globals cuz this shits getting ugly
        ip_textbox = Textbox((150, 150, 150), (WIDTH - textbox_width) // 2, HEIGHT //
                             2 - height_offset, textbox_width, textbox_height, textbox_height-8, "ip")
        port_textbox = Textbox((150, 150, 150), (WIDTH - textbox_width) // 2, HEIGHT //
                               2 + height_offset, textbox_width, textbox_height, textbox_height-8, "port")
        connect_button = text_button((150, 150, 150), (WIDTH - textbox_width) // 2, HEIGHT//2 +
                                     3*height_offset, textbox_width, textbox_height, textbox_height-8, "join game", None)
        while self.running:
